...
class PolynomialDataset(Dataset):
    def __init__(self, data_file, train=True, train_ratio=0.8):
        """
...
class PolynomialTrainer:
    ...
    def polynomial_verification_loss(self, pred_roots, coeffs, weight=0.1):
        ...
        for i in range(batch_size):
            ...
            for j in range(5):
                # 복소수 근 구성 (허수부, 실수부)
                imag = pred_roots[i, j*2]
                real = pred_roots[i, j*2 + 1]
                
                # 복소수 거듭제곱 계산 (PyTorch 텐서로)
                def complex_power_torch(real, imag, n):
                    result_real, result_imag = real, imag
                    for _ in range(n-1):
                        new_real = result_real * real - result_imag * imag
                        new_imag = result_real * imag + result_imag * real
                        result_real, result_imag = new_real, new_imag
                    return result_real, result_imag
                
                # 각 차수별 계산
                x2_real, x2_imag = complex_power_torch(real, imag, 2)
                x3_real, x3_imag = complex_power_torch(real, imag, 3)
                x4_real, x4_imag = complex_power_torch(real, imag, 4)
                x5_real, x5_imag = complex_power_torch(real, imag, 5)
                
                # f(x) = ax^5 + bx^4 + cx^3 + dx^2 + ex + f
                fx_real = a*x5_real + b*x4_real + c*x3_real + d*x2_real + e*real + f
                fx_imag = a*x5_imag + b*x4_imag + c*x3_imag + d*x2_imag + e*imag
                
                # |f(x)|^2 (방정식을 만족하면 0에 가까워야 함)
                equation_error += fx_real**2 + fx_imag**2
            
            verification_loss += equation_error
...
def test_model(model, test_loader, device, num_examples=5):
    """모델 테스트 및 결과 출력"""
    model.eval()
...
def main():
    ...
    # 데이터셋 생성
    train_dataset = PolynomialDataset(data_file, train=True)
    test_dataset = PolynomialDataset(data_file, train=False)
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    
    # 모델 생성
    model = PolynomialRootNet(hidden_size=512, num_layers=5, dropout=0.3)
    ...
    # 모델 테스트
    test_model(trainer.model, test_loader, device)
