...
def main():
    # 설정
    data_file = "polynomial_dataset_2.json"  # 또는 .json
    batch_size = 32
    epochs = 100
    learning_rate = 0.003
    num_layers_list = [4, 6, 8, 10, 12, 14, 16, 18, 20]  # 테스트할 num_layers 값들
    hidden_size = 1024
    dropout = 0.3
    ...
    for num_layers in num_layers_list:
        print(f"\n=== num_layers={num_layers}로 훈련 시작 ===")
        
        # 모델 생성
        model = PolynomialRootNet(hidden_size=hidden_size, num_layers=num_layers, dropout=dropout)
        ...
        results['final_test_loss'].append(trainer.best_test_loss)
        results['train_losses'].append(trainer.train_losses)
        results['test_losses'].append(trainer.test_losses)
        
        # 모델 테스트
        print(f"\n=== num_layers={num_layers} 테스트 결과 ===")
        test_model(trainer.model, test_loader, test_dataset, device)
        
        # 모델 저장
        trainer.save_model(f"best_polynomial_model_num_layers_{num_layers}", save_formats=['pth'])
    
    # 결과 시각화
    plot_comparison(results)

def plot_comparison(results):
    """num_layers에 따른 훈련 결과 비교 시각화"""
    # 1. 모든 num_layers의 훈련/테스트 손실 곡선
    plt.figure(figsize=(12, 6))
    
    for idx, num_layers in enumerate(results['num_layers']):
        plt.plot(results['train_losses'][idx], label=f'Train Loss (num_layers={num_layers})', alpha=0.6)
        plt.plot(results['test_losses'][idx], label=f'Test Loss (num_layers={num_layers})', linestyle='--', alpha=0.6)
    
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training and Test Loss for Different num_layers')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.yscale('log')
    plt.tight_layout()
    ...
    # 2. 최종 테스트 손실 비교 (막대 그래프)
    plt.figure(figsize=(8, 5))
    plt.bar(results['num_layers'], results['final_test_loss'], color='skyblue')
    plt.xlabel('Number of Layers')
    plt.ylabel('Final Test Loss')
    plt.title('Final Test Loss vs Number of Layers')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
