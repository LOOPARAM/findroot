...
class RootPredictorNet(nn.Module):
    """근 예측 신경망"""
    def __init__(self, input_dim=3, hidden_dims=[128, 256, 512, 256, 128], max_roots=10):
        ...
        # 근의 개수 예측 헤드 (0개부터 max_roots개까지)
        self.num_head = nn.Sequential(
            nn.Linear(hidden_dims[-1], 64),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(64, max_roots + 1),  # 0, 1, 2, ..., max_roots개 근
            nn.Softmax(dim=1)
        )
        ...
        self.values_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(64, max_roots)  # 최대 max_roots개 근의 값
        )
...
class RootPredictor:
    def __init__(self, csv_file_path, max_roots=10):
    ...
    def train(self, epochs=100, batch_size=128, learning_rate=0.001, validation_split=0.2):
        """모델 학습"""
        print("=== PyTorch 복잡한 초월방정식 근 예측 신경망 학습 시작 ===")
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                # 최고 모델 저장
                torch.save(self.model.state_dict(), 'best_root_model3.pth')
            else:
                patience_counter += 1
        ...
        # 최고 모델 로드
        self.model.load_state_dict(torch.load('best_root_model3.pth'))
...
if __name__ == "__main__":
    ...
    # 예측기 생성 (최대 10개 근까지 처리)
    predictor = RootPredictor(csv_file_path, max_roots=10)
    ...
    print(f"\n학습된 모델이 'best_transcendental_root_model.pth'에 저장되었습니다!")
